...
class shot:
  ...
  def __init__ ( self, match_info, event, players, teams ):
    ...
    self.MatchId = match_info.Id
    self.Region = match_info.Region
    self.Tournament = match_info.Tournament
    self.Season = match_info.Season
    self.X = event[ 'x' ]
    self.Y = event[ 'y' ]
  ...
  def get_header ( self ):
    return 'shot_id:match_id:region_id:tournament_id:season_id:minute:player_id:team_id:outcome_id:type_id:x:y'

  def get_content ( self ):
    return '%d %d %d %d %d %d %d %d %d %d %.2f %.2f\n' % ( int( self.Id ), int( self.MatchId ), self.Region, self.Tournament, self.Season, int( self.Minute ), int( self.PlayerId ), int( self.TeamId ), int( self.OutcomeId ), int( self.TypeId ), float( self.X ), float( self.Y ) )
